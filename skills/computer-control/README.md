...
- 在聊天和 AI 对话场景里，先补齐历史上下文，再逐轮验证后续交互
...
- 增加“聊天记录回看与逐轮验证”SOP，避免上一轮未结束就发送下一轮
- 增加 Windows 托盘恢复、遮挡恢复、微信托盘登录态恢复的双层策略

## `focus_window` 的语义边界

- `focus_window` 的成功应理解为“目标窗口稳定恢复为用户可见的前台窗口”。
- 如果窗口只是托盘驻留句柄、短暂前台后又回落、或者仍停留在屏幕外/最小化态，严格来说就不应算成功。
- 对托盘类应用，不要把 `focus_window` 当成万能恢复键；更稳定的路径通常是先走托盘 SOP，例如 `Win+B -> Enter`，必要时再配合方向键和截图确认。
- 如果应用需要通过托盘图标触发业务级恢复动作，应该优先走托盘恢复流程，而不是反复调用 `focus_window`。
- 如果 `focus_window` 返回成功，但目标窗口仍没在桌面上出现，也应直接按“系统托盘窗口”处理。
