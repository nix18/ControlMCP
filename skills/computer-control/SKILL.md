...
- 用最少截图完成最多验证
- 对聊天、问答、流式生成场景，先补齐上下文采集，再推进下一轮交互
...
- 如果 `focus_window(...)` 返回成功，但随后的截图/窗口检查里目标窗口仍然没有真实出现在桌面上，要把它视为“系统托盘窗口”，不要继续重试 `focus_window`
- 对这类“focus 后仍不出现”的窗口，默认恢复路径改为：从系统托盘点击/键盘进入，而不是继续依赖窗口句柄聚焦
...
对“focus 后不出现”的窗口，推荐恢复顺序：

```text
focus_window -> capture_window/capture_screen 验证
-> 仍未出现则标记为系统托盘窗口
-> Win+B -> wait(0.5) -> Enter
-> 必要时方向键定位托盘图标 -> Enter
-> capture_screen/capture_window 再验证
```
...
5. 判断窗口是否真的可操作：
   ...
   - `focus_window` 成功但桌面上仍看不到目标窗口：按“系统托盘窗口”处理，直接走托盘恢复路径
6. 判断当前任务属于哪种路径：
   - 全局导航：还没进目标应用/目标页面
...
聊天/问答类补充规则：
- 对聊天记录、AI 对话、工单时间线这类需要回看历史上下文的界面，优先先做一次长截屏，再决定是否继续发消息
- 当用户要求“基于刚刚的对话总结/判断/提取结论”时，不要只看当前可见窗口，必须先补采上方历史记录
- 如果当前界面只看到局部回复，不足以支撑总结，就继续滚动并再次长截屏，直到关键信息完整
...
1. 输入框是否已恢复可输入状态
2. 回复末尾是否不再继续增长
3. 是否出现推荐追问按钮、点赞/点踩等“回答已收口”信号
4. 只有确认本轮回答结束后，才能发送下一轮问题
...
适用：AI 对话、IM 聊天、客服窗口、评论流

```text
1. 聚焦聊天正文区域，而不是输入框或左侧会话列表
2. 用 capture_scroll_region(...) 对右侧正文做长截屏，先补齐历史上下文
3. 阅读长截屏，确认上一轮问题和回答是否完整
4. 如需继续追问：点击输入框 -> 粘贴问题 -> 发送
5. wait(1-3s) 后 capture_window/capture_region 复核是否开始生成
6. 再 wait 一轮并复核：末尾内容是否仍在增长、输入框是否恢复、是否出现收口信号
7. 仅当本轮回答确认结束后，才进入下一轮
8. 做最终总结前，再次长截屏回看关键历史记录，避免只依据最近可见片段下结论
```
...
修正：
- 把“本轮回复已收口”当作硬门槛，而不是参考信号
- 发送后至少做一次等待 + 一次窗口复核；长回复要做两轮复核
- 对聊天类任务，不要按固定秒数盲发后续问题

### 7. 总结前没有回看前文，只看当前可见区域

修正：
- 先对正文区做长截屏，再整理总结
- 总结、审校、复盘类任务默认要求回看历史上下文
- 如果中途出现跑偏回复，先定位跑偏点，再决定是否补发纠偏提示
